...
obsData_av <- obsData %>%
  ...
  tidyr::gather("VarName","ObsValue",FodderBeet.Leaf.LAI:Soil.Temperature.7) %>% # FIXME: find automated way of identifying variables and ignore factors when gathering
  mutate(VarName = factor(VarName)) %>%
...
mergedDF %>%
  ...
  geom_point(alpha=0.5, aes(colour=Irr,shape=Nit), size=1) +
