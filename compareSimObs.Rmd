...
```{r, echo=FALSE, include=TRUE}
# read observed data
obsData <- read.csv(paste0(rootPath,"/ObservedDataWorked_Biomass.csv"), header = TRUE)
summary(obsData)

# alternative with all data
pathDrive_K <- "K:/Rainshelter/Fodderbeet 2016-2017/Analysis" # FIXME: Send to powerPlant or need to run APSIM-X there?
obsData <- read.xlsx(paste0(pathDrive_K,"/Observed.xlsx"),sheetName = "OBS", header=TRUE)
...
obsData_av <- obsData %>%
  ...
  tidyr::gather("VarName","ObsValue",Soil.Temperature.1:Soil.SoilWater.SW.8) %>%
  mutate(VarName = factor(VarName)) %>%
  rename(SimName=SimulationName) %>%
  # FIXME: Do this in the initial script - correct names of array variables for merging
  mutate(VarName2 = ifelse(grepl('Soil.SoilWater', VarName),paste0(VarName,"."),as.character(VarName))) %>%
  mutate(VarName3 = ifelse(grepl('Soil.Temperature', VarName2),paste0(VarName2,"."),as.character(VarName2))) %>%
  mutate(VarName3=factor(VarName3)) %>%
  mutate(VarName = VarName3) %>%
  dplyr::select(-VarName2, -VarName3) # FIXME: Do it nicely later - the OR syntax did not work



unique(obsData_av$VarName)


# substrRight <- function(x, n){
#   substr(x, nchar(x)-n+1, nchar(x))
# }
# substrRight(colnames(df)[33],1)
# 
...
#   mutate(Nit=factor(Nit)) %>%
#   mutate(Clock.Today=ymd(Clock.Today)) %>%
#   dplyr::select(-Plot,-Rep,-Code, -Irr,-Nit) %>%
#   group_by(Clock.Today,SimulationName) %>%
#   summarise_all(funs(mean)) %>%
#   tidyr::gather("VarName","ObsValue",FodderBeet.Leaf.LAI:FodderBeet.AboveGround.Live.Wt) %>%
#   mutate(VarName = factor(VarName)) %>%
#   ungroup() %>%
#   rename(SimName=SimulationName)
# 
...
# FIXME: Merging is not working, many variables left behind
