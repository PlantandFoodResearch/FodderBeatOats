...
(ii) From file above, an observed file for APSIM-X format (i.e. each variable is a column)
...
pathFolderRaw <- "K:/Rainshelter/Fodderbeet 2016-2017/Analysis"
...
```{r}
head(obsData)
print(paste0("Save to ",pathFolderRaw))
write.csv(obsData, paste0(pathFolderRaw,"/ObservedDataWorked.csv"), row.names = FALSE)
```
...
pathFolderWork <- "C:/GitHubRepos/2017_FodderBeetOats/FodderBeatOats/Observed"  # FIXME: Send to powerPlant?

write.xlsx(df,paste0(pathFolderWork,"/ObservedDataWorked_APSIMX.xlsx"), sheetName="OBS", row.names = FALSE) 
```
